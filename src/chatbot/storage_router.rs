...
pub async fn append_thread(thread_id: &str, user_id: &str, content: Conversation) {
    match STORAGE {
        ...
        AvailableStorages::MongoDB => {
            super::mongodb_storage::append_thread(thread_id, user_id, content).await;
        }
    }
}

/// Reads a thread from the storage. Returns an error if the thread is not found, most likely because it doesn't exist.
pub async fn read_thread(thread_id: &str) -> Result<Conversation, std::io::Error> {
    match STORAGE {
        ...
        AvailableStorages::MongoDB => {
            ...
            match super::mongodb_storage::read_thread(thread_id).await {
