use super::types::Conversation;

/// Represents the possible available storage options for the threads
enum AvailableStorages {
    Disk,
    MongoDB,
}

/// The currently active storage for the threads
static STORAGE: AvailableStorages = AvailableStorages::Disk;

/// Appends a thread to the storage. User_Id is ignored for the disk storage.
pub fn append_thread(thread_id: &str, user_id: &str, content: Conversation) {
    match STORAGE {
        AvailableStorages::Disk => {
            super::thread_storage::append_thread(thread_id, content);
        }
        AvailableStorages::MongoDB => {
            super::mongodb_storage::append_thread(thread_id, user_id, content);
        }
    }
}

/// Reads a thread from the storage. Returns an error if the thread is not found, most likely because it doesn't exist.
pub fn read_thread(thread_id: &str) -> Result<Conversation, std::io::Error> {
    match STORAGE {
        AvailableStorages::Disk => super::thread_storage::read_thread(thread_id),
        AvailableStorages::MongoDB => {
            // super::mongodb_storage::read_thread(thread_id)
            // .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "Thread not found"))
        match super::mongodb_storage::read_thread(thread_id) {
            Some(thread) => Ok(thread.content),
            None => Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Thread not found")),
        }
        },
    }
}
